...
1. **Post-retrieval** — Compute ratios after `generate_dataset` in your training pipeline
2. **Downstream Feature View** — Create a second Feature View whose `feature_df` reads the first Feature View's output and computes derived columns

**Programmatic ratio generation with safe division:**
...
ratio_df = aggregated_fv_df  # output of the base aggregation Feature View
for num, denom, alias in RATIOS:
    ratio_df = ratio_df.with_column(
        alias,
