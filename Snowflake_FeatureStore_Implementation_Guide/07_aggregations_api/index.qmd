...
::: {.callout-note title="SDK Version and Documentation Status"}
...
See also the [platform capabilities and preview status table](../13_advanced_patterns/index.qmd#platform-capabilities-and-preview-status-march-2026) in Chapter 12.
:::
...
> 📂 **Chapter code:** [Browse companion scripts on GitHub](https://github.com/Snowflake-Labs/snowflake-featurestore-imp-guide/tree/main/Snowflake_FeatureStore_Implementation_Guide/07_aggregations_api/_code)
...
### Window offset {#sec-window-offset}
...
### 1. Use Consistent Naming Conventions {.unnumbered}
```python
...
### 3. Group Related Features and Consolidate Feature Views {#sec-consolidate .unnumbered}
...
### 4. Tile Sharing Across Feature Views {#sec-tile-sharing .unnumbered}
...
### ❌ Pitfall 5: Partial Leading-Edge Tiles from Incomplete Source Intervals {#sec-partial-interval-tiling}
...
::: {.callout-warning title="Do not use CURRENT_TIMESTAMP() inside a DT or feature_df"}
...
For non-tiled DT Feature Views with explicit window functions, use `ROWS BETWEEN N PRECEDING AND 1 PRECEDING` (excluding `CURRENT ROW`) as the DT-safe equivalent. See [Chapter 5: Partial Leading-Edge Interval](../05_feature_pipelines/index.qmd#sec-partial-interval) and [Chapter 6: Data Leakage from Partial Intervals](../06_temporal_features/index.qmd#sec-partial-interval-leakage).

---

## Data-Driven Feature Definitions {#sec-data-driven}
...
### Pattern 1: Categorical Pivot — Static vs Dynamic {.unnumbered}

[Filter Expressions Within Features](#sec-filter-expr) shows how `CASE WHEN` decomposes a metric by a single hard-coded dimension value. When the set of values is large or evolving, generate the columns programmatically:
...
### Pattern 2: Multi-Window Sweep {.unnumbered}

The [Multi-Window Features](#multi-window-features) section above shows a `build_features` helper that loops over columns and windows. This is already a data-driven pattern: the `columns` and `windows` lists are the configuration, and the helper generates `Feature` objects with consistent naming. Refer to that helper as the starting point for more complex patterns.

### Pattern 3: Metric × Dimension × Window Factory {.unnumbered}
...
### Pattern 4: Config-Driven Feature Specifications {.unnumbered}
...
### Pattern 5: Ratio and Derived Features {.unnumbered}
...
### Pattern 6: Interaction Features {.unnumbered}
...
### Static vs Dynamic — Trade-Off Summary {.unnumbered}
...
### Consuming Data-Driven Features with OBJECT_CONSTRUCT {.unnumbered}

The factory patterns above enforce a naming convention — `{DIMENSION}_{METRIC}_{AGG}_{WINDOW}` — that becomes a **queryable contract**. Snowflake's [`OBJECT_CONSTRUCT`](https://docs.snowflake.com/en/sql-reference/functions/object_construct) with `ILIKE` and `EXCLUDE` filtering can slice dynamically generated columns by any naming axis without hard-coding column lists:
...
## Tiled Feature Views and Online Serving {#sec-tiled-online}

::: {.callout-warning title="Tiled Feature Views Cannot Be Served Directly via Hybrid Table OFTs"}
...
### Workaround: `online_fv_from_tiled` Helper {#sec-tiled-online-workaround}
...
::: {.callout-note title="How the helper works"}
...
1. Converts each `Feature` aggregation type to its SQL equivalent (e.g., `Feature.sum` → `SUM`, `Feature.stddev` → `STDDEV`)
...
### Manual Approach: Direct SQL {#sec-tiled-online-manual}
...
::: {.panel-tabset group="feature-definition"}
...
::: {.callout-important title="Performance: FULL Refresh on Scoped Window"}
...
::: {.callout-tip title="Postgres Online Store (PubPr)"}
The **Postgres-backed online store** (`OnlineStoreType.POSTGRES`) handles tiled Feature Views with **query-time tile reassembly**. See [Chapter 9: Real-Time and Online Serving](../09_real_time_online_serving/index.qmd#sec-batch-postgres).
:::
...
Continue to [Chapter 8: Online Features](../08_online_features/index.qmd) to learn about Online Feature Tables for real-time serving, including the limitations and workarounds for tiled Feature Views discussed above.
