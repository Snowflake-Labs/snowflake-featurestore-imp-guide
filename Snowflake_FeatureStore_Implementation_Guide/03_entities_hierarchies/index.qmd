...
```python
# Typical order: define entities, register each, then build Feature Views
...
# Define an entity (register with fs.register_entity before use in Feature Views)
user_entity = Entity(
    name="USER",
    join_keys=["USER_ID"],
...
Rollup Feature Views (see [Chapter 07](../07_aggregations_api/index.qmd#sec-rollup)) aggregate features from a finer-grained entity to a coarser-grained entity using a **mapping DataFrame**. The design of this mapping table significantly affects both correctness and performance.
...
See [Chapter 06: PIT Correctness with Entity Mappings](../06_temporal_features/index.qmd#sec-mapping-leakage) for a detailed discussion.
:::
...
Continue to [Chapter 04: Feature Views](../04_feature_views/index.qmd) to learn about Feature View types, versioning, and lifecycle management.
