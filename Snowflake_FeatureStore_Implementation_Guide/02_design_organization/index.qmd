...
```{mermaid}
%%| fig-cap: "Single Feature Store: one database and schema with multiple feature views"
...
```{mermaid}
%%| fig-cap: "Domain-based organization: multiple schemas in one database"
...
```{mermaid}
%%| fig-cap: "Single vs multiple Feature Stores decision flow"
...
Even with multiple Feature Stores, the Snowsight Feature Store UI (**AI & ML → Features**) lets users switch between any tagged Feature Store schema via the dropdown and browse Feature Views, Entities, and lineage without writing code. This assists with the "cross-domain discovery" issue — teams can explore what exists across stores before creating new features. See [Chapter 12: Snowsight Feature Store UI](../12_operations/index.qmd#sec-snowsight-ui) for a visual walkthrough.
:::
...
```{mermaid}
%%| fig-cap: "Hybrid organization: shared Feature Store feeding domain-specific stores"
...
```{mermaid}
%%| fig-cap: "Schema-based DEV TEST PROD environments"
...
```{mermaid}
%%| fig-cap: "Database-based environments for dev test and prod"
...
```{mermaid}
%%| fig-cap: "Feature Store RBAC role hierarchy"
%%| fig-alt: "Account admin to sysadmin to FS admin dev and consumer roles"
flowchart TB
  AA[ACCOUNTADMIN] --> SYS[SYSADMIN]
  SYS --> AD[FS_ADMIN_ROLE]
  SYS --> DV[FS_DEV_ROLE]
  SYS --> CN[FS_CONSUMER_ROLE]
```
...
```{mermaid}
%%| fig-cap: "Feature promotion workflow across DEV TEST and PROD"
...
::: {.callout-important title="Two-layer management: physical + logical"}
DCM manages the **physical layer** — Dynamic Tables, Views, Tags, grants, warehouses. The **logical layer** — Entities as named constructs, Feature View metadata (descriptions, version labels, entity associations) — still requires the Feature Store Python SDK or manual TAG application via SQL (see [Appendix D: Tag Convention](../appendices/D_tag_convention/index.qmd)). In practice this means a DCM project handles object creation and promotion, while a lightweight deployment script (or CI step) calls `fs.register_feature_view()` or applies the TAG JSON to complete the logical registration. See [Chapter 13: Reconciling Desired vs Actual State](../13_advanced_patterns/index.qmd#sec-reconciliation) for patterns that bridge this gap.
:::
...
```{mermaid}
%%| fig-cap: "Start simple then scale practices"
...
This is not yet exposed through the Feature Store Python API's `FeatureView` constructor -- use `ALTER DYNAMIC TABLE` after registration. See the [DT warehouse documentation](https://docs.snowflake.com/en/user-guide/dynamic-tables/warehouse-selection) for details.
:::
...
**Solution**: Implement a shared Feature Store for common entities. Require feature review before creation. Use [feature discovery tools](../12_operations/index.qmd) to search existing features before creating new ones.
