...
WHERE TABLE_SCHEMA = 'FEATURE_STORE'
  AND (
      COLUMN_NAME LIKE '%SPEND%' 
      OR COLUMN_NAME LIKE '%PURCHASE%'
      OR COMMENT ILIKE '%purchase%'
  )
...
WHERE c.TABLE_SCHEMA = 'FEATURE_STORE'
  AND t.TABLE_TYPE IN ('DYNAMIC TABLE', 'VIEW')
  AND c.COLUMN_NAME NOT IN ('USER_ID', 'PRODUCT_ID', 'SESSION_ID', 'ORDER_ID')  -- Exclude keys
...
WITH feature_counts AS (
    SELECT 
        COLUMN_NAME,
        COUNT(DISTINCT TABLE_NAME) AS fv_count,
        LISTAGG(DISTINCT TABLE_NAME, ', ') AS feature_views
    FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA = 'FEATURE_STORE'
      AND COLUMN_NAME NOT IN ('USER_ID', 'PRODUCT_ID', 'SESSION_ID', 'ORDER_ID')
    GROUP BY COLUMN_NAME
)
SELECT *
FROM feature_counts
WHERE fv_count > 1
ORDER BY fv_count DESC, COLUMN_NAME;
