...
Use CRON when features should refresh on a **calendar schedule** aligned to upstream batch loads rather than continuous lag. This is common when source data arrives in known batches (e.g., a nightly ETL) and refreshing more frequently would add cost without improving freshness:
...
user_daily_session_df = session.sql("""
    SELECT
        ...
        MAX(START_TS) AS LAST_SESSION_TS
...
Prefer **V01**, **V02**, … for operational clarity and sorting. Other schemes (semantic versions, date stamps) are possible but should be agreed team-wide.
...
| **Consumers** | Point new pipelines to **V02**; migrate models and batch jobs on your timeline; retire **V01** only after usage drops to zero. |
...
This gives you the inner query only. For the full DDL of an already-registered DT (including `TARGET_LAG`, `WAREHOUSE`, etc.), use `SELECT GET_DDL('DYNAMIC_TABLE', 'FEATURE_STORE."MY_FV$V01"')`.
:::
...
versions = fs.list_feature_views(feature_view_name="USER_ORDER_FV")

# Get a specific version
v2_fv = fs.get_feature_view(name="USER_ORDER_FV", version="V02")
...
See [Chapter 03: Modeling at a Different Grain](../03_entities_hierarchies/index.qmd#sec-cross-grain) for entity design considerations and [Chapter 10: Training & Inference](../10_training_inference/index.qmd) for multi-entity spine design.
...
### 4. Version on Any Consumer-Visible Change
