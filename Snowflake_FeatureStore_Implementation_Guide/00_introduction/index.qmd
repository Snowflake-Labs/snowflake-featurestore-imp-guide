...
> 📂 **Chapter code:** [Browse companion scripts on GitHub](https://github.com/Snowflake-Labs/snowflake-featurestore-imp-guide/tree/main/Snowflake_FeatureStore_Implementation_Guide/00_introduction/_code)
...
### Verify Environment {#sec-verify-environment}
...
> 📁 **Full code:** [`_code/sample_data.py`](_code/sample_data.py)
...
- **DataFrames via `session.sql()`**: Feature View definitions, spines, and other DataFrames are primarily constructed using `session.sql("SELECT ...")` rather than the Snowpark DataFrame API. Both approaches produce identical lazy Snowpark DataFrames, but `session.sql()` is used throughout because SQL is more widely known and easier to read for the transformations involved. Where the Snowpark DataFrame API offers a notable advantage (e.g., programmatic column building), both forms are shown side by side. When you do rely on generated SQL (Snowpark, **dbplyr**, or similar), third-party pretty-printers and optional CTE hoists can make plans easier to read in Snowsight or `QUERY_HISTORY`; see [Appendix C](../appendices/C_snowpark_to_dynamic_table/index.md#sec-sql-formatting).
...
Each abbreviation is expanded on first use in every chapter. After that, the short form is used for readability.
...
Agreeing on feature naming conventions at the start of a project prevents inconsistency, reduces refactoring, and makes features discoverable across teams. The table above is a starting point — adapt it to your organization and enforce it through code review and automation. See [Chapter 2: Establish Naming Conventions Early](../02_design_organization/index.qmd) for object-level conventions (databases, schemas, Feature Views, Entities) and [Chapter 7: Naming Conventions](../07_aggregations_api/index.qmd) for aggregation-specific prefixing patterns.
:::
...
Continue to [Chapter 1: Concepts](../01_concepts/index.qmd) to learn about the core Feature Store objects and terminology.
