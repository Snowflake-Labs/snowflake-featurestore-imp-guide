...
![Online Feature Table: offline FV to Hybrid Table via stream and task](../_diagrams/png/dg_08_oft_hybrid.png){fig-alt="Offline feature view merges into hybrid table for low latency reads"}
...
This applies equally to **batch inference**. A DT-based scoring pipeline (see [Chapter 10: Continuous Batch Inference](../10_training_inference/index.qmd#sec-continuous-batch)) reads from upstream Feature Views that have their own refresh lag, compounded by the scoring DT's own `target_lag`. A Stream/Task/Stored Procedure pattern has the same issue: the Task schedule determines how often scoring runs, but the features it reads may already be stale by the time the Task fires.
...
![External serving: Feature Store to OFT or CDC to external store](../_diagrams/png/dg_08_external_serving.png){fig-alt="Offline feature store path to hybrid OFT or to external key value store"}
