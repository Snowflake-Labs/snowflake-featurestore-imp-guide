...
- Implement batch inference workflows, including DT-based continuous scoring
...
Use **open-source libraries** (scikit-learn, XGBoost, LightGBM, etc.) when your training data fits in memory, or **`snowflake.ml.modeling`** estimators when you need distributed fitting inside the warehouse. Both produce a trained object you can log with **`Registry.log_model()`**. See [Chapter 09: Preprocessing -- Pipeline Classes](../09_preprocessing/index.qmd#pipeline-classes-scikit-learn-vs-snowflake-ml) for guidance on choosing between the two.
...
| Memory | OOM errors; high disk spill | Consider Snowpark Optimised warehouse, or reduce batch size |
| Both | OOM + skew | Snowpark Optimised at larger size (3XL+) |

::: {.callout-tip title="Prefer standard Gen2 warehouses"}
For join-heavy dataset generation workloads, **standard Gen2 warehouses** typically outperform Snowpark Optimised warehouses of equivalent credit cost. A standard 4XL Gen2 (128 nodes × 16 GB) provides far more parallelism than an SPO 3XL (16 nodes × 256 GB) at similar cost. Only move to SPO when profiling confirms memory -- not parallelism -- is the constraint.
:::
...
3. **`COPY INTO` from a materialised table** -- if you need fine-grained control over Parquet file layout (row group size, compression).
...
If your Feature Views follow the recommended convention of aliasing their timestamp column to a standard name (e.g., `FV_TS` -- see [Chapter 06: timestamp_col Requirements](../06_temporal_features/index.qmd#sec-timestamp-col-requirements)), name the spine timestamp to match:

```python
# ✅ GOOD: Standardised timestamp name matches FV convention
spine = session.sql("""
    SELECT
        USER_ID,
