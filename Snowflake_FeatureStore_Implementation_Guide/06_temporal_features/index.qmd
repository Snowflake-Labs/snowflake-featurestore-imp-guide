...
Both `generate_training_set` and `generate_dataset` accept `include_feature_view_timestamp_col=True` (default `False`). When enabled, the output includes the Feature View's `timestamp_col` value for the ASOF-matched row alongside the spine timestamp. This is useful when you need to derive features that depend on the **gap** between the spine time and the feature time -- for example, "days since last order at prediction time":
...
training_df = training_df.with_column(
    ...
    F.datediff("day", F.col("LAST_ORDER_TS"), F.col("EVENT_TS"))
)
```
