...
> 📂 **Chapter code:** [Browse companion scripts on GitHub](https://github.com/Snowflake-Labs/snowflake-featurestore-imp-guide/tree/main/Snowflake_FeatureStore_Implementation_Guide/06_temporal_features/_code)
...
```{mermaid}
%%| fig-cap: "Data leakage vs point-in-time correctness"
%%| fig-alt: "Without PIT future data leaks into labels with PIT only past data is used"
flowchart LR
  subgraph GOOD["✅ With PIT"]
    direction TB
    L2[Label Jan 15 churn] --> F2[Features only through Jan 14] --> R2[Realistic metrics]
  end
  subgraph BAD["❌ Without PIT"]
    direction TB
    L1[Label Jan 15 churn] --> F1[Features include Jan 16-31 data] --> R1[Inflated metrics]
  end
  GOOD ~~~ BAD
```
...
```{mermaid}
%%| fig-cap: "ASOF join selects latest feature snapshot on or before spine time"
...
## Derived features and point-in-time correctness {#sec-derived-features}
...
**Pattern 1: Daily event snapshots** -- one row per user per active day. ASOF picks the most recent day <= spine TS.
...
```sql
ALTER DYNAMIC TABLE FEATURE_STORE_DEMO.FEATURE_STORE."USER_DAILY_EVENT_STATS$V01" REFRESH;
```
...
Dynamic Tables created directly via SQL in the Feature Store schema are not visible to `fs.list_feature_views()` or the Snowsight Feature Store UI unless the correct internal TAGs are applied. See [Appendix D: Tag Convention](../appendices/D_tag_convention/index.qmd) for the required TAG schema. This applies to all SQL-created objects in this section.
:::
