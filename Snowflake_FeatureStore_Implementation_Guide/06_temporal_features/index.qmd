...
::: {.callout-tip}
**Standardise `timestamp_col` with aliasing.** Just as entity join keys are aliased to a consistent name (see [Chapter 01](../01_concepts/index.qmd)), adopt a **standard timestamp column name** across all Feature Views -- for example `FV_TS`. Alias the source timestamp in the Feature View SQL:
...
# See Chapter 05 and Chapter 07 for details
```
...
::: {.callout-warning}
Adding a JOIN to a config table may affect the DT engine's ability to use incremental refresh, depending on the query pattern. Test `REFRESH_MODE` in `INFORMATION_SCHEMA.DYNAMIC_TABLES()` after registration to verify incremental behaviour is preserved. See [Chapter 05: Pitfall 2](../05_feature_pipelines/index.qmd) for diagnostics.
:::
...
### 2. Use Timestamps That Produce Multiple Rows Per Entity
...
### 4. Version Feature Views consistently
...
See [Chapter 05: Partial Leading-Edge Interval](../05_feature_pipelines/index.qmd#sec-partial-interval) for the full pipeline-level mitigation pattern and [Chapter 07](../07_aggregations_api/index.qmd#sec-partial-interval-tiling) for the tiling-specific safeguard.
