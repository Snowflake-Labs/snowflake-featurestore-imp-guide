...
The DataFrame API emits SQL that is sometimes dense or deeply nested. For review, debugging, or side-by-side comparison with hand-written SQL, use a dialect-aware formatter (for example **SQLGlot** with `pretty=True`, optionally hoisting derived tables to CTEs). Trade-offs, alternatives (SQLFluff, sqlfmt, IDE tools), and an R **dbplyr** note are covered in [Appendix C: Formatting machine-generated SQL](../appendices/C_snowpark_to_dynamic_table/index.md#sec-sql-formatting).
:::
...
- This is a **SQL-level capability**; the Feature Store Python API does not yet expose `RELY` primary keys or per-DT refresh mode overrides. Use `ALTER DYNAMIC TABLE` after API registration, or create the upstream stage as a plain DT outside the Feature Store (the same pattern recommended in [Section 3](../appendices/C_snowpark_to_dynamic_table/index.md) and the [DT-to-DT Chaining](#sec-chaining) section above for cases where the API lags behind SQL features).
- See [Understanding primary keys in dynamic tables](https://docs.snowflake.com/en/user-guide/dynamic-tables-primary-keys) for the full list of supported patterns.
:::
